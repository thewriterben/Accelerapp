...
class PerformanceProfiler:
    ...
    def _identify_hotspots(
        self, function_name: str, execution_time_ms: float, memory_mb: float
    ) -> List[Dict[str, Any]]:
        """
            ...
            hotspots.append(
                {
                    ...
                    "severity": (
                        "high" if execution_time_ms > self.slow_threshold_ms * 5 else "medium"
                    ),
                }
            )
    ...
    def _generate_recommendations(
        self, execution_time_ms: float, memory_mb: float, hotspots: List[Dict[str, Any]]
    ) -> List[str]:
        """
        ...
        if not recommendations:
            recommendations.append(
                "Performance within acceptable thresholds. No immediate action needed."
            )
    ...
    def compare_with_baseline(self, function_name: str) -> Dict[str, Any]:
        ...
        if not baseline:
            return {"error": "No baseline found for function", "function": function_name}
        ...
        if not current:
            return {"error": "No current profile found for function", "function": function_name}
        ...
        if baseline.memory_used_mb > 0:
            memory_change = (
                (current.memory_used_mb - baseline.memory_used_mb) / baseline.memory_used_mb * 100
            )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "slowest_functions": [
                {"name": p.function_name, "time_ms": p.execution_time_ms} for p in slowest
            ],
            "memory_intensive_functions": [
                {"name": p.function_name, "memory_mb": p.memory_used_mb} for p in memory_intensive
            ],
                ...
                regressions.append(
                    {
                        ...
                        "severity": (
                            "critical" if comparison["time_change_percent"] > 50 else "high"
                        ),
                    }
                )
    ...
    def optimize_function(self, function_name: str) -> Dict[str, Any]:
        ...
        if not profile:
            return {"error": "No profile found for function", "function": function_name}
