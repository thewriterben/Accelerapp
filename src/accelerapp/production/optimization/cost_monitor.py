...
class CostMonitor:
    ...
    def get_total_cost(
        self, provider: Optional[CloudProvider] = None, resource_type: Optional[ResourceType] = None
    ) -> float:
        """
    ...
    def identify_optimization_opportunities(self) -> List[Dict[str, Any]]:
        ...
        for usage in self.resources.values():
            provider_costs[usage.provider] = (
                provider_costs.get(usage.provider, 0) + usage.total_cost
            )
    ...
    def generate_cost_report(
        self, report_id: str, start_date: Optional[str] = None, end_date: Optional[str] = None
    ) -> CostReport:
        """
    ...
    def get_cost_breakdown(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_provider": {
                provider.value: self.get_total_cost(provider=provider) for provider in CloudProvider
            },
            "by_resource_type": {
                rt.value: self.get_total_cost(resource_type=rt) for rt in ResourceType
            },
    ...
    def apply_cost_optimization(self, opportunity: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if opp_type == "underutilized_resource":
            ...
            result["message"] = (
                f"Optimization type '{opp_type}' not supported for automatic application"
            )
