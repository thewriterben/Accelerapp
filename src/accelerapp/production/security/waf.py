...
class WebApplicationFirewall:
    ...
    def inspect_request(
        ...
        if source_ip in self.blocked_ips:
            self._record_detection(
                "ip-blacklist", ThreatLevel.HIGH, source_ip, endpoint, method, "IP is blacklisted"
            )
            return {
                "allowed": False,
        ...
        if not rate_limit_result["allowed"]:
            self._record_detection(
                "rate-limit", ThreatLevel.MEDIUM, source_ip, endpoint, method, "Rate limit exceeded"
            )
            return rate_limit_result

        # Check rules
        rules = sorted([r for r in self.rules.values() if r.enabled], key=lambda r: r.priority)

        for rule in rules:
            threat = self._check_rule(
                rule, source_ip, endpoint, method, headers or {}, body, query_params or {}
            )
            if threat:
                return threat
    ...
    def _determine_threat_level(self, rule_type: RuleType) -> ThreatLevel:
        ...
        high_severity = [RuleType.SQL_INJECTION, RuleType.XSS, RuleType.PATH_TRAVERSAL]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        detections_by_level = {"low": 0, "medium": 0, "high": 0, "critical": 0}
    ...
    def generate_security_report(self) -> Dict[str, Any]:
        ...
        high_threats = [
            d for d in self.detections if d.threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL]
        ]
        ...
        top_ips = sorted(top_attacking_ips.items(), key=lambda x: x[1], reverse=True)[:10]

        return {
            "summary": stats,
            "high_severity_threats": len(high_threats),
            "top_attacking_ips": [{"ip": ip, "attempts": count} for ip, count in top_ips],
            "recommendations": self._generate_recommendations(high_threats, top_ips),
        }

    def _generate_recommendations(
        self, high_threats: List[ThreatDetection], top_ips: List[tuple]
    ) -> List[str]:
        """Generate security recommendations."""
        recommendations = []

        if high_threats:
            recommendations.append(f"Review {len(high_threats)} high-severity threats immediately")
