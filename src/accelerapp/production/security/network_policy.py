...
class NetworkPolicyEnforcer:
                ...
                if self._rule_matches(
                    ...
                    if rule.action == PolicyAction.DENY:
                        self._record_violation(
                            policy.policy_id, source, destination, port, protocol, "DENIED"
                        )
                        return {
                            "allowed": False,
                        ...
                        self._record_violation(
                            policy.policy_id, source, destination, port, protocol, "LOGGED"
                        )

        # Default deny if no matching rule
        self._record_violation("default", source, destination, port, protocol, "DEFAULT_DENY")
    ...
    def _rule_matches(
        ...
        if rule.source_labels:
            if not all(source_labels.get(k) == v for k, v in rule.source_labels.items()):
                return False

        # Check destination labels
        if rule.destination_labels:
            if not all(destination_labels.get(k) == v for k, v in rule.destination_labels.items()):
                return False

        return True

    def _record_violation(
        self, policy_id: str, source: str, destination: str, port: int, protocol: str, action: str
    ):
    ...
    def list_policies(
        self, namespace: Optional[str] = None, enabled_only: bool = False
    ) -> List[NetworkPolicy]:
        """
    ...
    def get_violations(
        self, policy_id: Optional[str] = None, limit: int = 100
    ...
    def export_policy(self, policy_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "kind": "NetworkPolicy",
            "metadata": {"name": policy.name, "namespace": policy.namespace},
            "spec": {
    ...
    def _export_rules(self, rules: List[NetworkRule], rule_type: str) -> List[Dict[str, Any]]:
        """Export rules in Kubernetes format."""
        exported = []
        for rule in rules:
            rule_spec = {"ports": [{"port": p, "protocol": "TCP"} for p in rule.ports]}
            if rule.source_labels:
