...
class SecurityAuditSystem:
    ...
    def run_compliance_check(
        self, check_id: str, context: Optional[Dict[str, Any]] = None
    ) -> ComplianceResult:
        """
    ...
    def run_compliance_scan(self, standard: Optional[ComplianceStandard] = None) -> Dict[str, Any]:
        """
    ...
    def get_failed_authentications(
        ...
        """
        events = self.get_events(event_type=AuditEventType.AUTHENTICATION, actor=actor, limit=limit)
    ...
    def _get_top_actors(self, events: List[AuditEvent], limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"actor": actor, "event_count": count} for actor, count in top_actors]
    ...
    def _get_top_resources(self, events: List[AuditEvent], limit: int = 10) -> List[Dict[str, Any]]:
        ...
        top_resources = sorted(resource_counts.items(), key=lambda x: x[1], reverse=True)[:limit]

        return [{"resource": resource, "access_count": count} for resource, count in top_resources]
