...
class ABTestingFramework:
    ...
    def create_test(
        self, test_id: str, name: str, description: str, variants: List[Dict[str, Any]]
    ) -> ABTest:
        """
        ...
        test_variants = [
            TestVariant(
                name=v["name"], config=v["config"], weight=v.get("weight", 1.0 / len(variants))
            )
            for v in variants
        ]
    ...
    def get_results(self, test_id: str) -> Optional[Dict[str, Any]]:
        ...
        for variant in test.variants:
            variant_results = {"name": variant.name, "metrics": {}}
    ...
    def calculate_statistical_significance(
        self, test_id: str, metric_name: str, confidence_level: float = 0.95
        ...
        results = {"metric": metric_name, "confidence_level": confidence_level, "variants": []}
        ...
        for variant in test.variants:
            ...
            if values:
                ...
                results["variants"].append(
                    {"name": variant.name, "mean": mean, "sample_size": len(values)}
                )
