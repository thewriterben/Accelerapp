...
class ModelPerformanceAnalyzer:
            ...
            try:
                with open(metrics_file, "r") as f:
                    ...
                    for agent_name, measurements in data.items():
                        self.metrics[agent_name] = [PerformanceMetric(**m) for m in measurements]
            except Exception:
    ...
    def get_agent_performance(
        self, agent_name: str, task_type: Optional[str] = None, time_window: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        if time_window:
            ...
            measurements = [m for m in measurements if datetime.fromisoformat(m.timestamp) > cutoff]
        ...
        for metric_name in all_metric_names:
            values = [m.metrics[metric_name] for m in measurements if metric_name in m.metrics]
    ...
    def compare_agents(
        self, agent_names: List[str], metric_name: str, task_type: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        comparison = {"metric": metric_name, "task_type": task_type, "agents": []}
    ...
    def get_trend(
        ...
        measurements = [
            m
            for m in measurements
            if datetime.fromisoformat(m.timestamp) > cutoff and metric_name in m.metrics
        ]

        if not measurements:
            return {"agent_name": agent_name, "metric": metric_name, "trend": "no_data"}

        # Extract values with timestamps
        data_points = [
            {"timestamp": m.timestamp, "value": m.metrics[metric_name]} for m in measurements
        ]
        ...
        if n < 2:
            ...
            if denominator > 0:
                ...
                if slope > 0.01:
                    trend = (
                        "improving" if metric_name in ["accuracy", "success_rate"] else "degrading"
                    )
                elif slope < -0.01:
                    trend = (
                        "degrading" if metric_name in ["accuracy", "success_rate"] else "improving"
                    )
                else:
                    trend = "stable"
            else:
    ...
    def clear_old_metrics(self, days: int = 30) -> int:
        ...
        for agent_name in self.metrics:
            ...
            self.metrics[agent_name] = [
                m for m in self.metrics[agent_name] if datetime.fromisoformat(m.timestamp) > cutoff
            ]
