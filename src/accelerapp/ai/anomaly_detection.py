...
class AnomalyDetector:
    ...
    def detect_anomaly(
        self, device_id: str, metric_name: str, value: float, threshold_factor: float = 3.0
    ...
    def predict_failure(self, device_id: str, lookback_hours: int = 24) -> Dict[str, Any]:
        """
        ...
        """
        recent_anomalies = self.get_anomalies(device_id=device_id, time_window=lookback_hours)
        ...
        if failure_prob > 0.7:
            ...
            risk_level = "low"
            recommendations = ["Continue normal operation", "Monitor for pattern changes"]
        ...
        return {
            ...
            "most_affected_metric": (
                max(metrics_affected.items(), key=lambda x: x[1])[0] if metrics_affected else None
            ),
        }
    ...
    def clear_old_anomalies(self, days: int = 30) -> int:
        ...
        self.anomalies = [a for a in self.anomalies if datetime.fromisoformat(a.timestamp) > cutoff]
