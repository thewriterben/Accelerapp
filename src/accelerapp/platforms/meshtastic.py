...
class MeshtasticESP32Platform(BasePlatform):
    ...
    def _generate_main(
        self, device_name: str, features: List[str], radio_config: Dict[str, Any]
    ) -> str:
        """Generate main source file."""
        ...
        # These three are built above the f-string rather than inside it: each
        # contains a backslash-escaped quote, and a backslash inside an f-string
        # expression is a SyntaxError before Python 3.12 (PEP 701). The package
        # declares requires-python >= 3.8, so this module could not be imported
        # at all on 3.8-3.11 -- including four of the five versions CI tests.
        wifi_init = (
            'void initializeWiFi() { Serial.println("WiFi initialized"); }'
            if "wifi" in features
            else ""
        )
        bluetooth_init = (
            'void initializeBluetooth() { Serial.println("Bluetooth initialized"); }'
            if "bluetooth" in features
            else ""
        )
        gps_init = (
            'void initializeGPS() { Serial.println("GPS initialized"); }'
            if "gps" in features
            else ""
        )
