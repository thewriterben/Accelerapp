...
class JobQueue:
    ...
    def enqueue(self, job_id: str, job_data: Dict[str, Any], priority: str = "normal") -> bool:
        """
        ...
        queued_job = QueuedJob(
            priority=priority_value, job_data=job_data, timestamp=job_data["enqueued_at"]
        )
    ...
    def start_processing(self, processor: Callable[[Dict[str, Any]], Dict[str, Any]]):
        ...
        for i in range(self.max_workers):
            worker = threading.Thread(target=self._worker_loop, args=(processor,), daemon=True)
            worker.start()
