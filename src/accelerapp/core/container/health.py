...
class ServiceHealthMonitor:
    ...
    def register_service(
        ...
        self._health_status[name] = HealthCheckResult(HealthStatus.UNKNOWN, "Not yet checked")
    ...
    async def check_service_health(self, name: str) -> HealthCheckResult:
        ...
        if name not in self._services:
            return HealthCheckResult(HealthStatus.UNKNOWN, f"Service '{name}' not registered")
        ...
        try:
            ...
            result = HealthCheckResult(
                HealthStatus.UNHEALTHY, f"Health check failed: {e}", {"exception": str(e)}
            )
            self._health_status[name] = result
    ...
    async def _default_health_check(self, name: str) -> HealthCheckResult:
        ...
        if hasattr(service, "is_healthy"):
            ...
            return HealthCheckResult(
                HealthStatus.HEALTHY if is_healthy else HealthStatus.UNHEALTHY, "Basic health check"
            )
    ...
    def is_all_healthy(self) -> bool:
        ...
        """
        return all(result.status == HealthStatus.HEALTHY for result in self._health_status.values())
