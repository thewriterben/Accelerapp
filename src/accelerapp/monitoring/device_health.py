...
class DeviceHealthMonitor:
            ...
            try:
                with open(metrics_file, "r") as f:
                    ...
                    for device_id, measurements in data.items():
                        self.metrics[device_id] = [HealthMetric(**m) for m in measurements]
            except Exception:
    ...
    def get_metric_history(
        self, device_id: str, metric_type: Optional[str] = None, time_window: Optional[int] = None
    ) -> List[HealthMetric]:
        """
    ...
    def set_threshold(self, metric_type: str, warning: float, critical: float) -> Dict[str, Any]:
        """
    ...
    def clear_old_metrics(self, days: int = 30) -> int:
        ...
        for device_id in self.metrics.keys():
            ...
            self.metrics[device_id] = [
                m for m in self.metrics[device_id] if datetime.fromisoformat(m.timestamp) > cutoff
            ]
    ...
    def export_health_report(
        self, device_id: Optional[str] = None, time_window: int = 24
    ) -> Dict[str, Any]:
        """
        ...
        for dev_id in devices:
            ...
            metric_stats = defaultdict(
                lambda: {"min": float("inf"), "max": float("-inf"), "avg": 0, "count": 0}
            )
