...
class CommunicationPolicy:
    ...
    def allows_communication(self, protocol: Protocol, port: int) -> bool:
        """
        ...
        return protocol in self.allowed_protocols and port in self.allowed_ports
...
class NetworkSegmentationService:
    ...
    def create_segment(
        self, segment_id: str, zone: NetworkZone, description: str = ""
    ) -> DeviceSegment:
        """
        ...
        Returns:
            Created segment
        """
        segment = DeviceSegment(segment_id=segment_id, zone=zone, description=description)
    ...
    def check_communication_allowed(
        self, source_device: str, destination_device: str, protocol: Protocol, port: int
    ) -> bool:
        """
        ...
        if source_segment_id == dest_segment_id:
            ...
            for policy in self._policies.values():
                if (
                    policy.source_device == source_device
                    and policy.destination_device == destination_device
                ):
        ...
        if not self._is_zone_communication_allowed(source_segment.zone, dest_segment.zone):
            return False
    ...
    def _is_zone_communication_allowed(
        self, source_zone: NetworkZone, dest_zone: NetworkZone
    ) -> bool:
        """
    ...
    def list_policies(self, device_id: Optional[str] = None) -> List[CommunicationPolicy]:
        """
    ...
    def isolate_device(self, device_id: str) -> bool:
        ...
        isolated = False
        for policy in self._policies.values():
            if policy.source_device == device_id or policy.destination_device == device_id:
                policy.enabled = False
                isolated = True
        return isolated
