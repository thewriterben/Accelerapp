...
class MeshNetworkManager:
    ...
    def get_network_stats(self) -> Dict[str, Any]:
        ...
        signal_strengths = [
            node.signal_strength for node in online_nodes if node.signal_strength is not None
        ]
        ...
        network_density = (
            len(self.topology.edges) / total_possible_edges if total_possible_edges > 0 else 0
        )
    ...
    def send_message(
        self, source_id: str, dest_id: str, message: str, encrypted: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def get_message_history(
        ...
        if node_id:
            messages = [
                m for m in messages if m["source"] == node_id or m["destination"] == node_id
            ]
