...
class OTAController:
    ...
    def start_update(
        ...
        firmware_version = (
            firmware_path.stem.split("-")[1] if "-" in firmware_path.stem else "unknown"
        )
    ...
    def _update_via_wifi(
        self, device_id: str, firmware_path: Path, device_info: Optional[Dict[str, Any]]
    ) -> None:
        """
    ...
    def register_progress_callback(
        self, device_id: str, callback: Callable[[UpdateProgress], None]
    ) -> None:
        """
    ...
    def get_active_updates(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        return {
            device_id: progress.to_dict() for device_id, progress in self.active_updates.items()
        }
