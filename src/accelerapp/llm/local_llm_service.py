...
class LocalLLMService:
    ...
    def register_provider(self, backend: LLMBackend, provider: LLMProvider) -> None:
        """
    ...
    def generate(
        ...
        **kwargs,
        ...
        target_backend = backend.value if backend else self.active_backend
                    ...
                    if fallback_backend in self.providers:
                        try:
                            ...
                            if provider.is_available():
                                model_name = model or self.config.get(
    ...
    def get_available_models(self, backend: Optional[LLMBackend] = None) -> List[str]:
        """
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        """
        status = {"active_backend": self.active_backend, "providers": {}}

        for backend_name, provider in self.providers.items():
            try:
                status["providers"][backend_name] = provider.health_check()
            except Exception as e:
                status["providers"][backend_name] = {"status": "error", "error": str(e)}
