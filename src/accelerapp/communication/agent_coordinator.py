...
class AgentCoordinator:
    ...
    def register_agent(self, agent_id: str, name: str, capabilities: List[str]) -> None:
        ...
        self.agents[agent_id] = AgentStatus(
            agent_id=agent_id, name=name, capabilities=capabilities, status="idle"
        )
    ...
    def unregister_agent(self, agent_id: str) -> bool:
        ...
        if agent_id in self.agents:
            ...
            if self.message_bus:
                self.message_bus.publish(
                    sender="coordinator", topic="agent.unregistered", content={"agent_id": agent_id}
                )
            return True
        return False

    def find_capable_agent(self, required_capability: str) -> Optional[AgentStatus]:
        """
    ...
    def assign_task(self, task_id: str, agent_id: str, task_data: Dict[str, Any]) -> bool:
        """
        ...
        if self.message_bus:
            self.message_bus.publish(
                sender="coordinator",
                topic="task.assigned",
                content={"task_id": task_id, "agent_id": agent_id, "task_data": task_data},
            )

        return True

    def complete_task(self, task_id: str, agent_id: str, result: Any) -> None:
        """
        ...
        if agent_id in self.agents:
            ...
            if self.message_bus:
                self.message_bus.publish(
                    ...
                    content={"task_id": task_id, "agent_id": agent_id},
                )
