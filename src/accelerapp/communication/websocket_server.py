...
class WebSocketCollaborationServer:
    ...
    async def _handle_client(self, websocket, path):
        ...
        try:
            ...
            await self._broadcast_event(EventType.AGENT_CONNECTED, {
                'client_id': client_id,
            ...
            await self._broadcast_event(EventType.AGENT_DISCONNECTED, {
                'client_id': client_id,
                'timestamp': datetime.now().isoformat()
            })
    ...
    async def _process_message(self, client: WebSocketClient, message: str):
        ...
        try:
            ...
            if msg_type == 'register':
                ...
                await self._broadcast_event(EventType.CODE_CHANGED, {
                    'file_path': file_path,
                    'updated_by': client.agent_id or client.client_id,
                    'timestamp': datetime.now().isoformat()
                ...
                await self._broadcast_event(EventType.AGENT_STATUS, {
                ...
                await self._broadcast_event(EventType.TASK_STARTED, {
                    'task_id': task_id,
                    'started_by': client.agent_id or client.client_id,
                    'timestamp': datetime.now().isoformat()
                })
                ...
                await self._broadcast_event(EventType.TASK_COMPLETED, {
                    'task_id': task_id,
                    'timestamp': datetime.now().isoformat()
                })
                ...
                await self._broadcast_event(EventType.MESSAGE, {
    ...
    async def _broadcast_event(self, event_type: EventType, data: Dict[str, Any], exclude_client: Optional[str] = None):
        """
