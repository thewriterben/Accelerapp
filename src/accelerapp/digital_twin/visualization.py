...
class TwinVisualizer:
    ...
    def get_overview_dashboard(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connected_devices": sum(
                1 for state in all_states.values() if state.get("connected", False)
            ),
            "devices": [
                {
                    "device_id": device_id,
    ...
    def get_state_timeline(
        self, device_id: str, duration_minutes: int = 60
        ...
        filtered_history = [snapshot for snapshot in history if snapshot.timestamp >= cutoff_time]
