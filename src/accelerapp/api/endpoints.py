...
class CodeGenerationAPI:
    ...
    def __init__(self, accelerapp_core, host: str = "localhost", port: int = 8080):
        ...
        self.rate_limiter.set_rule('default', RateLimitRule(
            max_requests=100,
            time_window=3600  # 1 hour
        ))
        
        # LLM-specific rate limits (more restrictive)
        self.rate_limiter.set_rule('llm', RateLimitRule(
            max_requests=20,
            time_window=3600,  # 1 hour
            burst_size=5
        ))
    ...
    def _create_handler(self):
        ...
        class APIHandler(BaseHTTPRequestHandler):
            ...
            def _check_rate_limit(self, client_id: str, limit_type: str = 'default') -> bool:
                ...
                allowed, info = api_instance.rate_limiter.check_limit(
                    f"{limit_type}:{client_id}"
                )
            ...
            def do_GET(self):
                ...
                if path == '/health':
                    ...
                    platforms = [
                        'arduino', 'esp32', 'stm32', 
                        'raspberry_pi', 'raspberry_pi_pico', 'micropython'
                    ]
            ...
            def _handle_software_generation(self, data: Dict[str, Any], client_id: str):
                ...
                try:
                    ...
                    result = api_instance.core.generate_software(
                        language=language,
                        spec=spec
                    )
            ...
            def _handle_ui_generation(self, data: Dict[str, Any], client_id: str):
                ...
                try:
                    ...
                    result = api_instance.core.generate_ui(
                        framework=framework,
                        spec=spec
                    )
            ...
            def _handle_key_generation(self, data: Dict[str, Any], client_id: str):
                ...
                try:
                    ...
                    api_key = api_instance.api_key_manager.generate_key(
                        new_client_id,
                        permissions
                    )
