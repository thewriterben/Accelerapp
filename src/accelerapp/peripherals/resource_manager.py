...
class PeripheralResourceManager:
    ...
    def get_allocation(
        self, resource_type: ResourceType, resource_id: Any
    ) -> Optional[ResourceAllocation]:
    ...
    def suggest_dma_allocation(
        self, peripheral_type: str, data_rate: int
        ...
        if available is not None:
            return {
                ...
                "priority": priority,
                "recommendation": (
                    "DMA recommended for high throughput" if data_rate > 100000 else "DMA optional"
                ),
                "estimated_cpu_savings": min(
                    80, (data_rate / 10000) * 10
    ...
    def optimize_timer_allocation(self, pwm_channels_needed: int) -> Dict[str, Any]:
        ...
        """
        suggestions = {"timers_needed": 0, "allocations": [], "notes": []}
                ...
                suggestions["allocations"].append(
                    {
                        "timer": timer_id,
                        "channels": min(
                            channels_per_timer, pwm_channels_needed - i * channels_per_timer
                        ),
                    }
                )
            else:
                suggestions["notes"].append(
                    f"Insufficient timers available. Need {timers_needed}, only {i} available"
                )
                break
    ...
    def generate_resource_report(self) -> str:
        ...
        for resource_type in ResourceType:
            ...
            if allocations:
                ...
                for alloc in allocations:
                    lines.append(
                        f"  [{alloc.resource_id}] -> {alloc.peripheral_id} ({alloc.peripheral_type})"
                    )
