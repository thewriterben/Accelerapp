...
class EnvironmentalValidator:
    ...
    def validate_design(
        ...
        if not ip_adequate:
            issues.append(
                f"IP rating {design_ip} insufficient for {environment} (requires {required_ip})"
            )
        ...
        if not material_suitable:
            issues.append(
                f"Material {material} may not be suitable for {duration_months} months in {environment}"
            )
        ...
        if not temp_ok:
            issues.append(
                f"Design may not withstand temperature range: {env_spec['temperature_range']}"
            )
