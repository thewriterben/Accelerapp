...
class CYDMonitor:
    ...
    def _check_temperature(self, device_id: str, state: Dict[str, Any]) -> None:
        ...
        if temp >= self._thresholds["temperature_critical"]:
            ...
            self._create_alert(
                AlertLevel.WARNING, f"High temperature: {temp}°C", device_id, {"temperature": temp}
            )
    ...
    def _create_alert(
        self, level: AlertLevel, message: str, device_id: str, data: Dict[str, Any]
    ) -> None:
    ...
    def get_device_summary(self, device_id: str) -> Optional[Dict[str, Any]]:
        ...
        # Count alerts
        alerts_24h = len(
            self.get_alerts(device_id=device_id, since=datetime.now() - timedelta(hours=24))
        )

        unack_alerts = len(self.get_alerts(device_id=device_id, acknowledged=False))
